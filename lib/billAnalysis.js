...
// bill did not supply: the one place a rate is used (slabSaving) requires a
// verified per-unit rate from lib/tariffs.js and stays hidden until one exists.
import { CHARGE_LINES } from "./billData";
import { DOMESTIC_SLAB_BOUNDS, PROTECTED } from "./tariffs";
...
// Requires a VERIFIED marginal rate. While lib/tariffs.js rates are null this
// returns null and the module does not render — by design.
export function slabSaving(bill, slab, rates) {
  if (!slab || slab.isTop || !rates) return null;
  const marginal = rates.marginalRateFor?.(bill.unitsConsumed);
  if (marginal === null || marginal === undefined) return null;
  const unitsAboveBoundary = slab.unitsIntoSlab;
  if (!unitsAboveBoundary || unitsAboveBoundary <= 0) return null;
  return {
    unitsToDrop: unitsAboveBoundary,
    marginal,
    estimate: round(unitsAboveBoundary * marginal, 0),
  };
}
