...
// -------------------------------------------------- 4. protected status
//
// Eligibility is now a known rule rather than a guess: S.R.O. 1165(I)/2022,
// PART-II, A-1 Residential defines a protected consumer as a Non-ToU residential
// consumer at or below 200 kWh/month "consistently for the past 6 months".
//
// The Non-ToU condition matters more than the units for some households. A
// sanctioned load of 5 kW or above requires ToU metering and A-1(b) billing, so
// such a consumer is disqualified outright — and we can often read that straight
// off the bill, which beats computing a units-based answer that cannot apply.
export function protectedStatus(bill) {
  const units = bill?.unitsConsumed;
  const tariff = bill?.tariff;
  const loadKw = parseLoadKw(bill?.sanctionedLoad);
  if (units === null && !tariff && loadKw === null) return null;
  ...
  // ToU / 5 kW+ disqualification, derived only from what the bill states.
  const touByTariff = tariff ? /A-?1\s*\(?b\)?|\bTOU\b|TIME[- ]?OF[- ]?USE/i.test(tariff) : false;
  const touByLoad = loadKw !== null ? loadKw >= PROTECTED.touThresholdKw : false;
  const touExcluded = touByTariff || touByLoad;
  ...
  return {
    declared: touExcluded ? false : declared,
    units,
    threshold,
    qualifyingMonths: PROTECTED.qualifyingMonths,
    definition: PROTECTED.definition,
    source: PROTECTED.source,
    sourceUrl: PROTECTED.sourceUrl,
    carryForwardRule: PROTECTED.carryForwardRule,
    regainIsImplication: PROTECTED.regainIsImplication,
    // why, so the card can explain rather than just assert
    touExcluded,
    touReason: touExcluded
      ? touByLoad
        ? `sanctioned load of ${loadKw} kW`
        : "a Time-of-Use tariff code"
      : null,
    loadKw,
    touThresholdKw: PROTECTED.touThresholdKw,
    overThreshold,
    marginToThreshold,
    atRisk:
      !touExcluded && declared === true && marginToThreshold !== null &&
      marginToThreshold >= 0 && marginToThreshold <= threshold * 0.1,
  };
}

// "2 kW", "5.5 KW", "0.5" -> number of kW, or null when unreadable.
function parseLoadKw(v) {
