...
// Tiny in-memory TTL cache so repeat lookups of the same reference don't re-hit
// PITC. (Serverless instances are warm-reused; for cross-instance caching in
// production, back this with Vercel KV / Redis.)
const BILL_TTL_MS = 5 * 60 * 1000;
const BILL_CACHE_MAX = 300;
const billCache = new Map();

function cacheGet(key) {
  const hit = billCache.get(key);
  if (hit && Date.now() - hit.t < BILL_TTL_MS) return hit.html;
  if (hit) billCache.delete(key);
  return null;
}
function cacheSet(key, html) {
  if (billCache.size >= BILL_CACHE_MAX) billCache.delete(billCache.keys().next().value);
  billCache.set(key, { t: Date.now(), html });
}

// Public entry: cached + retried.
export async function fetchBillHtml(disco, ref, searchBy = "refno") {
  ...
  const key = `${disco}:${searchBy}:${ref}`;
  const cached = cacheGet(key);
  if (cached) return cached;
  const html = await withRetry(() => fetchBillOnce(disco, ref, searchBy));
  cacheSet(key, html);
  return html;
}
...
async function fetchBillOnce(disco, ref, searchBy = "refno") {
  ...
  const r1 = await fetch(url, { headers: { "User-Agent": UA }, cache: "no-store" });
  ...
  // HTML) OR a valid HTML page that just says "Bill Not Found!". Reject both —
...
export async function detectDisco(reference, searchBy = "refno") {
  ...
  try {
    const res = await fetch(
      ...
      { headers: { "User-Agent": UA, Accept: "application/json", "X-Requested-With": "XMLHttpRequest" }, cache: "no-store" }
    );
  ...
  try {
    return await Promise.any(
      probeList.map((code) => fetchBillHtml(code, reference, searchBy).then(() => ({ disco: code })))
    );
  } catch {
    return null;
  }
