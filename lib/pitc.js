...
const FETCH_DEADLINE_MS = 25000; // overall budget per lookup; uses most of the 30s route maxDuration (PITC is slow)
...
export async function fetchBillHtml(disco, ref, searchBy = "refno") {
  ...
  // One deadline across all retries and sub-requests, so a slow or hung PITC
  // fails fast with our own error page instead of a raw Vercel 504 timeout.
  const signal = AbortSignal.timeout(FETCH_DEADLINE_MS);
  const html = await withRetry(() => fetchBillOnce(disco, ref, searchBy, signal));
  await cacheSet(key, html, BILL_TTL);
  return html;
}

// Official rendered bill HTML via the ASP.NET Post/Redirect/Get flow (one attempt).
async function fetchBillOnce(disco, ref, searchBy = "refno", signal) {
  disco = disco.toLowerCase();
  if (!DISCOS[disco]) throw new Error("unknown company");
  const url = `${BILL_BASE}/${disco}bill`;
  const jar = {};

  const r1 = await fetch(url, { headers: { "User-Agent": UA }, cache: "no-store", signal });
