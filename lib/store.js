...
// Privacy: we deliberately store NO reference numbers and NO IP addresses, only
// a timestamp, the company (disco), the source page and the outcome. This keeps
// the dashboard useful without holding personal data.
...
export async function logBillCheck({ disco, page, outcome } = {}) {
  const ev = {
    t: Date.now(),
    disco: disco || "auto",
    page: page || "(direct)",
...
export async function getStats() {
  if (redis) {
    try {
      const [events, byDisco, byPage, byDay, total] = await Promise.all([
      ...
      return { configured: true, total: 0, byDisco: {}, byPage: {}, byDay: {}, recent: [] };
    }
  }
  // dev fallback (in-memory, not persistent)
  const byDisco = {}, byPage = {}, byDay = {};
  ...
  return { configured: false, total: memEvents.length, byDisco, byPage, byDay, recent: memEvents.slice(0, 50) };
}
